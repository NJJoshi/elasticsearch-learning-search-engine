...
...
```
- Use this below curl command to upload the file content. Use the port you have mapped for elasticsearch. This command will work on window command prompt as well.
```curl
...
```
- Use below mentioned command for windows on Powershell
```
Invoke-RestMethod -Uri "http://localhost:9201/products/_bulk" -Method Post -ContentType "application/x-ndjson" -InFile "products-bulk-upload.ndjson" 
```